...
export default class Dropdown extends PureComponent {
    ...
    onSelect(index) {
        ...
        let label = labelExtractor(data[index], index);
        ...
        setTimeout(() => this.onClose(label), delay);
    }
    ...
    renderItem({ item, index }) {
        ...
        return (
            <DropdownItem index={index} {...props}>
                <AppText style={[styles.item, itemTextStyle, textStyle]} numberOfLines={1}>
                    {title}
                </AppText>
            </DropdownItem>
        );
        ...
        return (
            <View onLayout={this.onLayout} ref={this.updateContainerRef} style={containerStyle}>
                <TouchableWithoutFeedback {...touchableProps}>
                    <View pointerEvents='box-only' style={{flexDirection:'row', alignItems:'center', justifyContent:'space-between'}}>
                        {/*{this.renderBase(props)}*/}
                        {/*{this.renderRipple()}*/}
                        <AppText style={{fontSize:16, color:'#333'}}>{value}</AppText>
                        <Icon name={'chevron-down'} size={28} style={{marginTop:4, color:'#c8c8c8'}} />
                    </View>
                </TouchableWithoutFeedback>
